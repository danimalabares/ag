...
\subsection*{The Kernel of the Homomorphism}
...
\subsection*{Rank-1 Condition and Algebraic Geometry}
...
\subsection{Dani's earlier attempt}
...
\section{November 26: Fano 3-folds (Sergey)}

\href{https://github.com/danimalabares/mirror-symmetry}{Here's} what happened today.

\section{November 29}

\subsection{Blowing up the cuspidal cubic curve (Dani)}

\begin{thing4}{Exercise 4.10}\leavevmode
	Let $Y$ be the cuspidal cubic curve $y^2=x^3$ in $\mathbb{A}^2$. Blow up the point $O=(0,0)$, let  $E$ be the exceptional curve, and let $\tilde{Y}$ be the strict transform of $Y$. Show that $E$ meets $\tilde{Y}$ in one point, and that $\tilde{Y}\cong\mathbb{A}^1$. In this case the morphism  $\varphi:\tilde{Y}\longrightarrow Y$ is bijective and bicontinuous, but it is not an isomorphism.
\end{thing4}

\begin{proof}[Solution]\leavevmode
First recall that the blow-up of $\mathbb{A}^2$ at the origin is the variety $X \subset \mathbb{A}^2_{x,y}\times\mathbb{P}^1_{t,u}$ defined by the equation $xu=ty$. We denote  $\varphi:X\longrightarrow \mathbb{A}^2$ the projection of $X$ onto the first factor. 

Now put the cuspidal cubic curve $Y$ given by $y^2=x^3$ inside $\mathbb{A}^2$ and consider its strict transform $\tilde{Y}$, which is the closure of  $\varphi^{-1}(Y-O)$. It turns out to be useful to consider the inverse image of $Y$ in  $X$ because it will contain $\tilde{Y}$ and $E$; it is given by the equations
\[xu=ty,\qquad y^2=x^3.\]
Take an affine chart $t=1$. Substituting, we get $x^2u^2=x^3$, that is, $x^2(x-u^2)=0$. So there are two irreducible components, the first given by $x=0=y$ and  $u$ free, and the second by $x=u^2$. The first component is the exeptional curve $E$, and the second is $\tilde{Y}$. They intersect when $u=0$. {\color{4}Is this enough to show that $\tilde{Y}$ and $E$ intersect at one point only?}

Now in the other affine chart $u=1$, we get  $x^2=t^2x^3$, so, $x^2(t^2x-1)$. The component $t^2x=1$ does not intersect $E$.

It remains to show that $\tilde{Y}$ has no singularities, since $Y$ being singular would prevent them from being isomorphic. What are the defining equations for $\tilde{Y}$? By the argument above it is clear that a point $\tilde{Y}$ satisfies $x=u^2$, $xu=y$ and $y^2=x^3$ when $t=1$.  The curve $y=u^3$ has no singularities, as $y^2=x^3$ when $x,y \neq 0$.

Likeways, when $u=1$, points in  $\tilde{Y}$ satisfy $t^2x=1$, $x=ty$ and  $y^2=x^3$, all of which are nonsingular. To make sure why, we may differentiate with respect to $t$, which yields $2tx=0$…

{\color{4}But all points satisfying these equations are in $\tilde{Y}$? I would like to know what is the ideal defining $\tilde{Y}$.} So maybe a projective ideal of $\mathbb{A}^2\times \mathbb{P}^1$?
\end{proof}

\begin{thing3}{After exposition}\leavevmode
	Here's a more algebraic way to show the restriction of the projection to the strict transform of $Y$ cannot be an isomorphism. The coordinate ring of $Y\subset \mathbb{A}^2$ is $\dfrac{k[x,y]}{\left<y^2=x^3\right>}$, and the coordinate ring of $\tilde{Y}$ is $k[u]$, which means that we are taking the chart  $u\neq 0$. Then we have a map
	\begin{align*}
		\psi: k[x,y] &\longrightarrow k[u] \\
		x &\longmapsto u^2\\
		y &\longmapsto u^3
	\end{align*}
	and of course we also have the projection $k[x,y]\twoheadrightarrow \frac{k[x,y]}{\left<y^2,x^3\right>}$. We have argued by the universal property of (?) that
	\[\begin{tikzcd}
		k[x,y]\arrow[r]\arrow[d, two heads]&  k[u]\\
		\frac{k[x,y]}{\left<y^2,x^3\right>}\arrow[ur,"\not\exists? f",dashed, swap]
	\end{tikzcd}\]
	We have made two remarks:
	\begin{enumerate}
	\item That $f $ is injective
	\item  That $\operatorname{img}f=k[u^2,u^3]$ which is a subring of  $k[u]$ but not equal to it. So it cannot be an isomorphism.
	\end{enumerate}
\end{thing3}

\subsection{There exists a unique generic point (Bruno)}

$X$ scheme, $Z \subset X$ irreducible closed subset, then there exists a unique $\xi \in X$ such that $\overline{\{\xi\}}=Z$, a \textit{\textbf{generic point of $X$}}.

First suppose that $X= \operatorname{Spec}(A)$. Choose $\xi \in X$. Then $\overline{\{\xi\}}=V(\xi)$. Because $\xi$ is a maximal ideal! $V(\xi)=\{ a \in \operatorname{Spec}(A):a> \xi\}$.

And uniqueness: $V(\xi_1)=V(\xi_2)\implies \xi_1=\xi_2$.

And in general we just use the affine cover of a scheme. And then the intersections might not be affine, but their intersections have affine covers.

\subsection{Some more Fano (Sergey)}

There exists only two proofs that an abstract fano $F$ has $\pi_{1}(F)=0$.

\begin{enumerate}
\item Uses DG/RG (diff/riemannian geometry?). Uses  Yau (existence of a nice metric), compactness. This was already explained in this, last semester.

	\[\mathbb{F}_p \longleftrightarrow \mathbb{Z}_p \hookrightarrow \mathbb{Q}_p \subset \overline{\mathbb{Q}}_p \subset ?\]

\item Uses $\mathcal{F}_p$, 1980s, Mori. Mori emphasized the role of rational curves.
	
\end{enumerate}

\section{December 3: some more blow-ups (Dani)}

I'll start by doing the same I did last time, that way we can see how well understood it is.

\begin{thing4}{Exercise 5.6}[Blowing up curve singularities]\leavevmode
\begin{enumerate}[label=(\alph*)]
\item Let $Y$ be the cusp of node of the following figure:
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\textwidth]{fig2}
	\end{figure}
	The node is $xy=x^6+y^6$ and the cusp is $x^3=y^2+x^4+y^4$
		\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\textwidth]{fig3}
	\end{figure}
	Enough of that. Show the strict transform of either of them is nonsingular.

	\begin{proof}[Solution]\leavevmode
	\begin{enumerate}[label=(\roman*)]
		\item Consider first the node $xy=x^6+y^6$. Before you do anything ask yourself why is this curve singular (this is exercise 5.1). The definition of a curve being \textit{\textbf{nonsingular}} at a point is that the rank of the matrix $\left( \frac{\partial f_i}{\partial x_j} \right)\Big|_{p}$ is $n-r$ where  $Y \subset \mathbb{A}^n$ is of dimension $r$ and its ideal is generetad by $f_i$. For our nodal curve at zero such a matrix is zero.

		%	The idea (see \href{https://en.wikipedia.org/wiki/Singular_point_of_a_curve#Algebraic_curves_in_the_plane}{wiki}) is that the polynomial is a function $\mathbb{A}^2\to k$, then by the implicit function theorem, given the equation $f(x,y)=0$, at least at the origin,  we can express either $x$ and $y$ as a smooth function of the other variable when the partial derivatives are not zero. The point is: take all partial derivatives and make sure \textit{they are not all zero} at the point where the could be a singularity. (Some day argue why this actually works.) This curve has vanishing partial derivatives at zero. 

			Now we mimic last week's exercise. The inverse image of $Y$ is given by that and $xU=y Y$ in the blow-up (which is contained in $\mathbb{A}^2\times \mathbb{P}^1$. Take the affine piece $T \to 1, U \to u$. We get  $xu=y$, and then  $x^2(u-x^4+x^6u^6)=0$. Now if $x=0$, also  $y=0$. This means the component corresponding to $x^2$ is again $E$. Now at the points of $\varphi^{-1}(Y)$ where $x=y=0$ we must also have $u=0$, meaining that, like before, they intersect at the point  $[0,T]$. This could be the only place where $\tilde{Y}$ is singular. Now look for the matrix $\left( \frac{\partial f_i}{\partial x_j} \right)\Big|_{p}$ and check its rank.
			
		The equations defining $ \tilde{Y}$ in this affine chart are $f_1=xu-y$ and  $f_2=u-x^4+x^6u^6$. Then the Jacobian matrix of $\tilde{Y}$ is
		\[\begin{pmatrix} \partial_xf_1&\partial_yf_1&\partial_uf_1\\
		\partial_xf_2&\partial_yf_2&\partial_uf_2\end{pmatrix}_{(0,0)}=\begin{pmatrix} u&-1&x\\ -4x^3 +6x^5u^6 & 0 &1+x^6\end{pmatrix}_{(0,0)}  \]
		which has rank 2 at $(0,0)$.

		By the way, according to ChatGPT, in this affine chart the strict transform looks as follow:
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.6\textwidth]{fig8}
		\end{figure}

	\item The case for the cusp $x^3=y^2+x^4+y^4$ is analogous.
	\end{enumerate}
\end{proof}

\item Now for a curve $Y \subseteq \mathbb{A}^2$ given by $f(x,y)=0$ define the  \textit{\textbf{multiplicity of $P \in Y$}} to be the least $r$ such that $f_r \neq 0$ for a decomposition $f=f_0+f_1+\ldots+f_d$ into homogeneous polynomials $f_i$ of degree $i$ which are called \textit{\textbf{tangent directions}} after making a change of coordinates that sends $P$ to zero. And define a \textit{\textbf{node}} to be a point of multiplicity 2.

	If $P$ is anode on a plane curve $Y$, show that $\varphi^{-1}(P)$ consists of two distinct nonsigunlar points on $\tilde{Y}$.

\begin{proof}[Solution]\leavevmode
Suppose $Y$ is given by $f=f_2+f_3+\ldots+f_d$. \textit{We may suppose} that $f_2=xy$ under a suitable change of coordinates. Also suppose that $P=(0,0)$. And there it goes again: the inverse image of  $Y$ is given by  $xU-y T=0$ and $xy-f'(x,y)=0$. Do  $T \to 1$ and $U\to t$. We get $xu=y$ and $x^2u=f'(x,xu)$, so $x^2(u-g(x,xu))$. We get two irreducible components… $\tilde{Y}$ is given by $u=-g(x,xu)$. This means  $\tilde{Y}$ meets $E$ at zero, so we look for a singularity here. So the defin
ing equations for $\tilde{Y}$ are $xu-y$ and  $u-g(x,ux)$. The Jacobian matrix is of rank 2 because $\partial_x g=0$ since it has degree more than 2.
\end{proof}
\item Let $P \in Y$ be the tacnode, which is the middle one here:
	\begin{figure}[H]
		\centering
		\includegraphics[width=.3\textwidth]{fig4}
		\includegraphics[width=.3\textwidth]{fig5}
		\includegraphics[width=.3\textwidth]{fig6}
		\caption*{$x^2=x^4+y^2\qquad\qquad \qquad  $ $x^2=x^4+y^4\qquad\qquad \qquad  $ $x^2=x^4+y^6$}
	\end{figure}
	Show it is a node.
\begin{proof}[Solution]\leavevmode
We just do everything again. By $T \to 1$ and $U\to u$ we get $\tilde{Y}$ given by $1-x^2-x^2u^4$ so actually when $x=y=0$ these curves don't meet. So take the other affine chart. Now you get  $\tilde{Y}$ given by $t^2-y^2t^4-y^2$ so they do meet at zero. (We can see the point  in this chart.) Evaluating at the origin we see that there remains a term $t^2$.\end{proof}
...
\item Finally show that
	\begin{figure}[H]
		...
		\caption*{$y^3=x^5$}
	\end{figure}
has a singularity that requires three blow-ups to be resolved. (Which is strange it looks quite smooth everywhere.)

\begin{proof}[Solution]\leavevmode
Blowing up gives $x^3(u^3-u^2)$ at the affine chart $T\to 1$, everything working nicely. Then $\tilde{Y}$ is a node, so as in item (b) we conclude that one more blow-up resolves the singularity.
\end{proof}
\end{enumerate}
\end{thing4}
...
\begin{thing4}{Proposition V.3.8}\leavevmode
	Let $C$ be an irreducible curve in the surface $X$. Then there exsits a finite sequence of monoidal transformations (=blow-ups) (with suitable centers) $X_n\to X_{n-1}\to\ldots \to X_1 \to X_0=X$ such that the strict transform $C_n$ of $C$ on $X_n$ is nonsingular.
\end{thing4}

\begin{thing4}{Definition on page 388}\leavevmode
	Let $C$ be an effective Cartier divisor on the surface $X$, and let $f$ be a local equation for $C$ at the point $P$. Then we define the \textit{\textbf{multiplicity}} of $C$ at $P$, denoted by $\mu_P(C)$ to be the largest integer $r$ such that $f \in \mathfrak{m}_P^r$ where $\mathfrak{m}_P \subset \mathcal{O}_{P,X}$ is the maximal ideal.
\end{thing4}

Remember that

\begin{thing4}{Theorem I.5.1}\leavevmode
	Let $Y\subseteq \mathbb{A}^n$ be an affine variety. Let  $P \in Y$ be a point. Then $Y$ is nonsingular at $P$ (Jacobian definition) iff the local ring $\mathcal{O}_{P,Y}$ is a regular local ring (=$\dim_k \mathfrak{m}/\mathfrak{m}^2=\dim \mathcal{O}_{P,Y}$, $\mathfrak{m}$ maximal ideal).
\end{thing4}

So that's algebra but the point is that multiplicity says how serious is the singularity $\leftrightsquigarrow$ how many blow-ups you will need to resolve it.

\begin{thing3}{Remark V.3.5.2}\leavevmode
	$\mu_P(C) =1$ iff $C$ is nonsingular at $P$, because in that case $\mathfrak{m}_{P,C}$ will be a principal ideal, so $f  \not \in \mathfrak{m}^2_{P,X}$.
\end{thing3}

\begin{thing6}{Proposition V.3.6}\leavevmode
	Let $C$ be an effective divisor on $X$, let $P$ be a point of multiplicity $r$ on $C$, and let $\pi: \tilde{X}\to X$ be the monoidal transformation with center $P$. Then
	\[\pi^*C=\tilde{C}+rE.\]
\end{thing6}

\begin{thing7}{Corollary V.3.7}\leavevmode
	With the same hypotheses, we have $\tilde{C}.E=r$, and $p_a(\tilde{C})=p_a(C)-\frac{1}{2}r(r-1)$.
\end{thing7}

So, I think that the fact the sequence of blow-ups must end is basically because the arithmetic genus $p_a$ is nonnegative by an exercise of part III.
...
\section{November 12: finding equations for $f(X)$ (Dani)}
My search shows that \href{https://math.stackexchange.com/questions/4158529/computing-the-image-of-a-projective-variety}{here} might be the answer.

\begin{exercise}[Sergey]\leavevmode
	Consider the map  $F:X\subset \mathbb{P}^2\longrightarrow \mathbb{P}^2$ $x,y\mapsto x^d,y^d$. This map puts a curve $X$ such as $x^2+y^2=1$ (circle) or $x+y=1$ (line) in $ \mathbb{P}^2$. What is the degree of the image? Is it singular?
\end{exercise}
\begin{proof}[Solution]\leavevmode
Consider the case when $d=3$. Define
 \begin{align*}
	f: \mathbb{P}^2 &\longrightarrow \mathbb{P}^2 \\
	(x:y:z) &\longmapsto (x^3:y^3:z^3)
\end{align*}
Let $X=V(x^2+y^2=z^2)$. We claim that $f(X)=V\Big((x^2+y^2-z^2)^3-27(xyz)^2\Big)$. One contention is immediate: if $(x:y:z)\in X$ then $f(x:y:z)=(x^2:y^2:z^2)$ satisfies
\end{proof}
$\mathsf{OK}$ since that is very hard I will proceed to the abstract proof for schemes.

\begin{proof}[Solution by Sergey]\leavevmode
	Consider roots of unity $\rho^d$. Let $z=x^d$ and $w=y^d$. Then we have that $z^{2/d}+w^{2/d}-1=0$. The solution reads
	\[\prod_{a=0}^{d-1}\prod_{b=0}^{d-1}\Big(\rho^az^{2/d}+\rho^bw^{2/d}-1\Big)=0\]
Compute this in a computer. Looks like for $d=3$ we get
\[w^6 + 3 w^4 (z^2 - 1) + 3 w^2 (z^4 + 7 z^2 + 1) + (z^2 - 1)^3.\]
This was computed with the following line on WolframAlpha:
\begin{quotation}
	\texttt{Product[Product[e\^{}(a * 2 * pi * i / 3) * z\^{}(2/3) + e\^{}(b * 2 * pi * i / 3) * w\^{}(2/3) - 1, {b, 0, 2}], {a, 0, 2}]}
\end{quotation}
...
Recall that in a past exercise by Dani (later also done by Victor) we studied the case of how a homomorphism of rings $\varphi:A\to B$ may or may not induce a map $Proj(B)\to Proj(A)$---we saw that the natural definition taking a prime homogeneous ideal $\mathfrak{p}\subset B$ to $\varphi^{-1}(\mathfrak{p})$ doesn't always work because such a preimage may not be a relevant ideal.

We proved that there is a certain open set where the induced map may be defined, which coincides with $\operatorname{Proj}(B)$ if the ring homomorphism is surjective. Also in the case when $\varphi$ is surjective, we showed that the induced map in $\operatorname{Proj}$ is a closed immersion. The following proposition states the analogue result for $\operatorname{Spec}$:
\begin{thing4}{Proposition II.2.3 }\leavevmode
\begin{enumerate}
	\item [(b)] If $\varphi:A\to B$ is a homomorphism of rings, then $\varphi$ induces a natural morphism of locally ringed spaces
		\[(f,f^\#):\Big(\operatorname{Spec}(B),\mathcal{O}_{\operatorname{Spec}(B)}\Big)\longrightarrow\Big(\operatorname{Spec}(A),\mathcal{O}_{\operatorname{Spec}(A)}\Big).\]
	\item [(c)] If $A$ and $B$ are rings, then any morphism of locally ringed spaces from $\operatorname{Spec}(B)$ to $\operatorname{Spec}(A)$ is induced by a homomorphism of rings $\varphi:A \to B$ as in (b).
	\item [(d)] Prove the converse to (c), namely, if  $f:Y\to X$ is a homeomorphism onto a closed subset, and $f^\#:\mathcal{O}_X\to f_*\mathcal{O}_Y$ is surjective, then $\varphi$ is surjective.
\end{enumerate}
\end{thing4}

Right, so we consider a map of schemes locally, that is, a map $f:Y:=\operatorname{Spec}(B) \to X:=\operatorname{Spec}(A)$, so that we have a map
\[\phi:A \to B.\]
This gives a well-defined map
\[A \longrightarrow A/\ker \phi\longrightarrow B\]
which in turn induces a morphism from $Y=\operatorname{Spec}(B)$ to a subset of $X=\operatorname{Spec}(A)$, $\operatorname{Spec}(A/I)$. In fact,  $\operatorname{Spec}(A/I) =V(I)$ since the prime ideals of $A/I$ are the ideals of $A$ that contain $I$.

Now if we suppose $f$ is algebraic we get that $f(X)$ is closed and onto, so that by item (d) we get that and that the map of rings is surjective.  This shows that it is enough to compute $I$.

\begin{thing4}{Remarks}\leavevmode
	\begin{itemize}
	\item Looks like the construction may be taken to $\operatorname{Proj}$, I only didn't find the statement that a map of $\operatorname{Proj}$ induces a map of rings. Indeed,
	\item how is the map of rings defined? And,
	\item how to take this to the practical difficulty of finding the equations for the above projective map? In StackExchange we are suggested to compute the ideal \textit{locally}. All we have to do is find the generators! So take the affine chart $\{z=1\}$, what is $B$ if  $\operatorname{Spec}(B) =X=V(x^2+y^2-1)$? Is it now just $k[x,y]/(x^2+y^2-1)$
	\item Another question is how to compute the degree of the image. This remains rather mysterious.
	\end{itemize}
\end{thing4}
...
\section{November 8: extension of coherent sheaves (Victor)}

\begin{thing4}{Exercise II 5.15}\leavevmode
	Let $X$ be a Noetherian scheme (has an open affine cover $U_i$ such that $\mathcal{O}_X(U_i)$ is Noetherian).  Let $U$ be an open subset and $\mathcal{F}$ a coherent sheaf on $U$. Then there is a coherent sheaf $\mathcal{F}'$ on $X$ such that  $\mathcal{F}'|_{U}\cong\mathcal{F}$.
\end{thing4}

\section{December 3}
\subsection{Blowing up a nonsingular subvariety (Victor)}

\begin{question}[Dani]\leavevmode
	Why should we be interested in blowing up a nonsingular subvariety?
\end{question}

\begin{exercise}\leavevmode
	Let $X$ be a nonsingular variety, let $Y$ be a singular subvariety of codimension $r \geq 2$, let $\pi:\tilde{X}\to X$ be the blowing-up of $X$ along $Y$, and let $Y'=\pi^{-1}(Y)$.
\end{exercise}

\begin{question}[dani]\leavevmode
	What is the definition of blowing-up along a subvariety?
\end{question}

In Chapter II we find the definition of blowing up with respect to an arbitrary sheaf of ideals. To get there, first we must define de $\operatorname{Proj}$ of a sheaf of graded algebras $\mathcal{S}$ over a scheme $X$ (so far we have only defined $\operatorname{Proj}$ for a graded ring). And then we must define a projective space bundle associated to a locally free sheaf $\mathcal{E}$.

\subsection{Grassmanians}

Take a linear section of a Grassmanian \[\underbrace{\mathbb{P}^{2-c}\cap\operatorname{Gr}(k,N)}_{X_C} \hookrightarrow  \mathbb{P}^{\binom{N}{k}-1}\]
for some $C\geq 0$. Compute its canonical bundle $\omega_{X_C}$ 

Easier question: for which $k,N$ is  $\operatorname{Gr}(k,\mathbb{R}^N)$ orientable? So we want to know what is its canonical bundle $\omega_{\operatorname{Gr}(k,N)}$.

Let's review \textit{\textbf{Plücker embedding}}. It is
\begin{align*}
	\operatorname{Gr}(k,V) &\longrightarrow \mathbb{P}(\Lambda^{k}V) \\
	U:=\left<u_1,\ldots,u_k\right>\subset V &\longmapsto [u_1\wedge\ldots\wedge u_k]\\
	U&\longmapsto \Lambda^{k}(U)=[\det U]
\end{align*}

\begin{exercise}[Raffaele's paper]\leavevmode
	Now take  vector space $\dim W=n+2$, classify all tuples $A^\alpha \in \Lambda^{2}( W)$, $\alpha=1,\ldots,n$, such that (Einsetin convention) $\phi_{\beta\alpha} A^\beta \wedge A^\alpha=0$ for some nondegenerate symmetric  $\phi$s, meaning $\phi_{\beta \alpha}=\phi_{\alpha \beta}$ and $\det\phi_{\alpha \beta}=0$.
\end{exercise}

We have taken the conditions on the $\phi$s to the following form. Consider
\[\begin{tikzcd}
	\mathbb{C}\arrow[r,"\phi"]&  S^2U\arrow[r,"S^2A"]&S^2\Lambda^{2}W\arrow[d,"\pi"]\\
	&  &  \Lambda^{4}W
\end{tikzcd}\]
where $\pi$ is a map between two representations, one irreducible and the other one not. Then the conditions on the $\phi$s become
\[\pi \circ S^2A\circ \phi=0.\]
I'm not sure wether this solved the problem.

We have concluded that
\[\omega_{\operatorname{Gr}(k,N)}=\mathcal{O}(-N),\qquad  \operatorname{Pic}(\operatorname{Gr}(k,N))=\mathbb{Z}\mathcal{O}(1).\]
Which come from Plücker embedding. So, telling if $\operatorname{Gr}(k,\mathbb{R}^N)$ is orientable reduces to checking that Plücker is a nontrivial bundle in Grassmanian.

\section{December 10: Kähler Differentials and the Canonical Sheaf}

Here's ChatGPT's today discussion summary:

Let \( X=\operatorname{Spec}(A) \) be an affine algebraic variety over a field \( k \), and let \( A = \mathcal{O}(X) \) denote its coordinate ring (the ring of global sections of its structure sheaf). The sheaf of Kähler differentials \( \Omega_X \) can be defined using the concept of Kähler differentials of \( A \) over \( k \).

\subsection{Definition of the Kähler Differential Module}

The module of Kähler differentials of \( A \) over \( k \) is denoted by \( \Omega_{A/k} \). It is defined by a universal property: there exists a map
\[
d: A \to \Omega_{A/k}
\]
that satisfies the Leibniz rule:
\[
d(ab) = a \cdot d(b) + b \cdot d(a) \quad \text{for all } a, b \in A.
\]
This map \( d \) is \( k \)-linear and describes how functions in \( A \) vary infinitesimally.

\subsection{Formal Partial Derivatives}

If \( A = k[x_1, \dots, x_n] \) is a polynomial ring, then \( \Omega_{A/k} \) can be explicitly generated by \( dx_1, \dots, dx_n \) with relations given by:
\[
d(f) = \sum_{i=1}^n \frac{\partial f}{\partial x_i} dx_i,
\]
where \( \frac{\partial f}{\partial x_i} \) are the formal partial derivatives of \( f \) with respect to \( x_i \). These derivatives are computed in the polynomial sense, without requiring analytic smoothness.

\subsection{The Sheaf of Kähler Differentials}

When working with a general algebraic variety \( X \) (not just affine), \( \Omega_X \) is defined by gluing these local constructions. Specifically:
\[
\Omega_X = \Omega_{A/k}
\]
locally on affine open subsets of \( X \).

\subsection{Geometric Meaning}

The sheaf \( \Omega_X \) encapsulates the infinitesimal variation of functions on \( X \). It serves as the algebraic geometric analog of the sheaf of holomorphic differential forms in complex geometry.

\subsection{Summary}

The sheaf of Kähler differentials \( \Omega_X \) is the sheafification of \( \Omega_{A/k} \) over \( X \) and provides insight into the local differential structure of the variety \( X \).
...
\section{An exercise on divisors by Lucas}

\begin{exercise}\leavevmode
Let $C$ be a complete smooth curve over $k=\bar{k}$ of genus $g$.
\begin{enumerate}[label=(\alph*)]
\item If $D \in \operatorname{Div}(C)$ is effective, show that $\operatorname{div}|D|\leq d=\operatorname{deg}D$ and equality iff $D=0$ or $g=0$.
\item If $C$ has $g=2$, show that $D$ is very ample iff $d \geq 5$. 
\item If $C\hookrightarrow \mathbb{P}^n$ with degree $D \leq n$ nondegenerate, show that $D=n$, $g=0$ and $C$ differs from the rational normal curve of degree $D$ by an automorphism of $\mathbb{P}^n$. 
\item Chacterize curves of degrees 2, 3 and 4 in $\mathbb{P}^n$. 
\end{enumerate}
\end{exercise}

\begin{proof}[Solution]\leavevmode
\begin{enumerate}[label=(\alph*)]
\item  by Riemann Roch we have that
	\[h^0(D)-h^1(D)=d-g+1\]
	We know that $\dim|D|=h^0(D)-1$ because $|D|=\mathbb{P}(C,\mathcal{O}_X(D))$ so $\dim|D|=h^1(D)+d-g$. $h^1(D)=h^0(-D+K)\leq h^0(K)=g$ using Serre duality. This gives $\dim|D|\leq d$.
...
\begin{thm}[Nakai-Moishezon Criterion for surfaces]\leavevmode
...
\begin{thing4}{Idea}\leavevmode
	Looks like the K3 surface should contain an elliptic curve (genus 1). There must be a theorem saying that this cannot happen when the surface is diffeomorphic to a quartic (this has to do with non-very ampleness of a line bundle (prop. 1.1 \cite{huyk3}). Showing that there exists an elliptic curve is probably equivalent to showing that there is a vector $E\in\operatorname{Pic}$ with $E^2=0$ and $L.E=1$.
\end{thing4}

We start considering a general elliptic K3 surface.

\begin{thing3}{Definition 1.1}[\cite{huyk3}, ch. 11]\leavevmode
	An \textit{\textbf{elliptic K3 surface}} is a K3 surface $X$ together with a surjective morphism $\pi:X\to \mathbb{P}^1$ such that geometric generic fibre is a smooth integral curve of genus one or, equivalently, if there exists a closed point $t \in \mathbb{P}^1$ such that $X_t $ is a smooth integral curve of genus one.
\end{thing3}

\begin{thing4}{Proposition 1.3}[\cite{huyk3}, ch. 11]\leavevmode
	Let $X$ be a K3 surface over an algebraically closed field $k$ with $\operatorname{char}(k)\neq 2,3$. Then $X$ admits an elliptic fibration if and only if there exists a non-trivial line bundle $E$ with $E^2=0$.
\end{thing4}

This gives me $E^2$ for free. Now I should find $L$ with  $L^2=4$.

That $E.L=0$ may be prop. 10.1.6  \cite{huyk3}.

\section{December 6}

\subsection{Riemann hypothesis for curves (Arthur)}

Riemann hypothesis is a statement related to the \textit{\textbf{zeta function}} 
\[\dseta(s)=\sum_{n\geq 1}\frac{1}{n^{-s}},\qquad \operatorname{Re}s>1.\]
Such a function has a an analogue in finite fields. We shall prove the analogue of the Riemann hypothesis for curves, which is the statement that
\[|\# C(\mathbb{F}_q)-\# \mathbb{P}'(\mathbb{F}_q)|\leq  2g \sqrt{q} \]
where $C$ is a smooth curve of genus $g$ over the field  $\mathfrak{F}_q$, and $C(\mathbb{F}_q)$ is the number of points of $C$ rational over $\mathbb{F}_q$.

The proof relies of on the computation of some intersection numbers. For the first one, we have shown that two varieties intersect transversally, so that their intersection number is exactly the size of their intersection as a set. Indeed,

\begin{thing4}{Theorem V1.1}\leavevmode
	There is a unique pairing $\operatorname{Div}X \times \operatorname{Div}X \to \mathbb{Z}$ denoted by $C.D$ for any two divisors $C,D$ such that
	 \begin{enumerate}
	\item if $C$ and $D$ are nonsingular curves meeting transversally, then $C.D=\#(C \cap D)$, the numebr of points of $C \cap D$,

	\item[2-4] symmetric, additive, depends only on linear equivalence.
	\end{enumerate}
\end{thing4}
...
\begin{defn}\leavevmode
	$D,E\subset S$ \textit{\textbf{intersect transversally}} at $P \in D \cap E$ if
	\[\mathfrak{m}_P=\left<\mathfrak{m}_{P,D},\mathfrak{m}_{P,E}\right>\]
	which reminds us of the fact that transversality means that $T_PS=T_PD+T_PE$. The condition above is equivalent: recall that $T_PS=\left( \frac{\mathfrak{m}_P}{\mathfrak{m}_P^2}^\vee \right) $. The equality $T_PS=T_PD+T_PE$ ammounts to showing that a linear functional on $T_PS$ vanishes iff it vanishes on both  $T_PD$ and $T_PE$. This is clear from the fact that $\mathfrak{m}_P=\left<\mathfrak{m}_{P,D},\mathfrak{m}_{P,E}\right>$.
\end{defn}
